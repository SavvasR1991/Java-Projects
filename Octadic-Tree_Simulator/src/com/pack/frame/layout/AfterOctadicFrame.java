...
public class AfterOctadicFrame {
	
	private int pixel;
	public JFrame frame;

	public AfterOctadicFrame(int dimension,ColorInputTable inputColor) {
		this.pixel = 20;
		InputFrameCreation(dimension,inputColor);
	}
	
	public void InputFrameCreation(int dimension,ColorInputTable inputColor) {
		int i,j;
		int frameSize = dimension*this.pixel+(dimension+1)*pixel;
		int pixelGap = (frameSize-dimension*this.pixel)/(dimension+1);
		JFrame frame = new JFrame("Output Colors ");
		
		Dimension Screnndimension = Toolkit.getDefaultToolkit().getScreenSize();
	    int x = (int) ((Screnndimension.getWidth()/2 - frame.getWidth()) / 2 +Screnndimension.getWidth()/2);
		...
		JLabel label = new JLabel("After Octadic Tree");		
